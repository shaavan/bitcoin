...
extern const std::string NET_MESSAGE_OTHER;
typedef std::map<std::string, uint64_t> mapMsgSize; //command, total bytes
...
public:
    ...
    uint64_t nSendBytes;
    mapMsgSize mapSendBytesPerMsg;
    uint64_t nRecvBytes;
    mapMsgSize mapRecvBytesPerMsg;
...
private:
    ...
    mapMsgSize mapSendBytesPerMsg GUARDED_BY(cs_vSend);
    mapMsgSize mapRecvBytesPerMsg GUARDED_BY(cs_vRecv);
};
