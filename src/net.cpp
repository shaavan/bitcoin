...
void CNode::CopyStats(CNodeStats& stats)
{
    ...
    {
        LOCK(cs_vSend);
        X(mapSendBytesPerMsg);
        X(nSendBytes);
    }
    {
        LOCK(cs_vRecv);
        X(mapRecvBytesPerMsg);
        X(nRecvBytes);
    }
    X(m_permissionFlags);
    if (m_tx_relay != nullptr) {
    ...
    while (msg_bytes.size() > 0) {
        ...
        if (m_deserializer->Complete()) {
            ...
            if (reject_message) {
                ...
                mapRecvBytesPerMsg.at(NET_MESSAGE_OTHER) += msg.m_raw_message_size;
                continue;
            }

            // Store received bytes per message
            // to prevent a memory DOS, only allow valid messages
            auto i = mapRecvBytesPerMsg.find(msg.m_type);
            if (i == mapRecvBytesPerMsg.end()) {
                i = mapRecvBytesPerMsg.find(NET_MESSAGE_OTHER);
            }
            assert(i != mapRecvBytesPerMsg.end());
      ...
      nLocalServices(nLocalServicesIn)
{
    ...
    for (const std::string &msg : getAllNetMessageTypes())
        mapRecvBytesPerMsg[msg] = 0;
    mapRecvBytesPerMsg[NET_MESSAGE_OTHER] = 0;
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    size_t nBytesSent = 0;
    {
        ...
        pnode->mapSendBytesPerMsg[msg.m_type] += nTotalSize;
